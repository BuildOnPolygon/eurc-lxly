...
contract DeployInit is Script {
    ...
    address l1EURC = vm.envAddress("ADDRESS_L1_EURC");
    address zkEURC = vm.envAddress("ADDRESS_L2_EURC"); // ATTN: needs to be deployed beforehand zkEURC
    address zkBwEURC = vm.envAddress("ADDRESS_L2_WEURC");
    ...
    /// @notice the L2 address that is able to upgrade the MinterBurner and NativeConverter's proxy contracts' implementation contracts
    address l2Admin = vm.envAddress("ADDRESS_L2_PROXY_ADMIN");
    /// @notice the L1 address that is able to pause and unpause the MinterBurner and NativeConverter contracts
    ...
    function run() external {
        ...
        L1Escrow l1Escrow = LibDeployInit.initL1Contracts(
            l1Owner, l1Admin, l2NetworkId, bridge, l1EscrowProxy, minterBurnerProxy, l1EURC
        );
        vm.stopBroadcast();
        ...
        vm.selectFork(l2ForkId);
        vm.startBroadcast(vm.envUint("DEPLOYER_PRIVATE_KEY"));
        (MinterBurner minterBurner, NativeConverter nativeConverter) = LibDeployInit.initL2Contracts(
            l2Owner,
            l2Admin,
            l1NetworkId,
            bridge,
        ...
        vm.selectFork(l2ForkId);
        console.log("MinterBurner owner=%s", minterBurner.owner());
